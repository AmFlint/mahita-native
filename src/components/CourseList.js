...
class CourseList extends Component {
  ...
  getVisibleCourses() {
    ...
    return this.props.courses.filter(course => {
      ...
      visible =
        visible && (category.includes(course.categorie) || !category.length);

      // One or more classes chosen
      visible =
        visible &&
        (!classe.length ||
          classe.reduce((acc, clas) => {
  ...
  render() {
    ...
    if (!this.state.dimensions) {
      return (
        <View
          style={{ flex: 1, width: maxWidth * 0.8, alignSelf: "stretch" }}
    ...
    return (
      <ScrollView style={styles.container}>
        <FlatList
          ...
          data={listData}
          renderItem={({ item }) => (
            <CourseItem itemWidth={itemWidth} course={item} />
          )}
